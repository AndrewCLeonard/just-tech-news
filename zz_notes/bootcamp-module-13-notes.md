...
| controller        | controllers    | n/a            | `index.js`       | <ul><li>sets up Express.js server routes for `apiRoutes` (`*/api`) and `homeRoutes` (`*/`) </li></ul>                                                                                                                                                                                                                                        |
| controller        | controllers    | n/a            | `home-routes.js` | all of the user-facing routes: homepage and login page                                                                                                                                                                                                                                                                                       |
| controller        | routes         | api            | `index.js `      | Hook up the server<ul><li>keep API endpoints organized</li><li>collect all API routes and package them up</li><li>often starts small, but allows for scalability</li><li>often used to give API endpoints a prefixed name, e.g. `router.use('/users', userRoutes);`</li></ul>                                                                |
| controller        | routes         | n/a            | `index.js`       | <ul><li>similar purpose to `routes/api/index.js`</li><li>collecting the packaged group of API endpoints, and prefixing them with the path `//api`.</li><li>second use of `router.use()` is a RESTful API practice. Allows us to return 404 error if/when making a request to incorrect resource (an endpoint that doesn't exist). </li></ul> |
| controller        | root folder    | n/a            | `server.js`      | <ul><li>turns on connection to Sequelize db and server</li><li>creates routing to PORT</li></br><li>because routes set up using the `index.js` files, no need to import multiple files for different endpoints. </li></ul>                                                                                                                   |
| models/controller | `models`       | n/a            | `index.js`       | imports `User` model and exports an object with it as a property.                                                                                                                                                                                                                                                                            |
